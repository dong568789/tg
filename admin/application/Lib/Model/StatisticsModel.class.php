...
class StatisticsModel extends Model
{
	...
	public function getTodayData($userid, $channelid, $gameid){
		...
		// 找出 昨天 支付成功的 注册渠道
		// group by 去重复，只取出一条
		$channelquery = $model->query("select * from yx_all_pay where create_time >= '".$starttime."' and status = 1 group by regagent order by id desc");
		foreach ($channelquery as $row) {
			$channellist[] = $row["regagent"];
		}

		// 找出 昨天 支付成功的 支付
		$dataquery = $model->query("select * from yx_all_pay where create_time >= '".$starttime."' and status = 1 order by id desc");
		foreach ($dataquery as $row) {
			$data[]=$row;
		}

		// 找出 昨天 登录sdk的 渠道
		$activechannelquery = $model->query("select agent,userid from yx_sdk_logininfo where login_time >= '".$starttime."' AND agent > '' group by agent");
		foreach ($activechannelquery as  $row) {
			$channellist[]=$row["agent"];
		}

		// 找出 昨天 登录的 sdk
		$activequery = $model->query("select agent,userid from yx_sdk_logininfo where login_time >= '".$starttime."' AND agent > '' GROUP BY userid,agent");
		$itemUserid = array();
		foreach ($activequery as $row) {
			$active[]=$row;
			$itemUserid[]=$row['userid'];
		}

		// 找出 昨天 注册玩家的 渠道
		$userchannelquery = $model->query("select agent from yx_all_user where reg_time >= '".$starttime."' group by agent");
		foreach ($userchannelquery as $row) {
		...
		// 取出所有渠道（用户，游戏，渠道）
		$sourcequery = $model->query("select * from yx_tg_source S left join yx_tg_game G on S.gameid = G.gameid left join yx_tg_channel C on S.channelid = C.channelid ".$conditionstr." order by S.id desc");
		$itemSource = array();
		foreach ($sourcequery as $row) {
			$source[]=$row;
		...
		if(!empty($itemSource)){
			...
			$strUid = '\''.str_replace(',', '\',\'', implode(',', $itemUserid)).'\'';
			 $before_login_query = $model->query("select userid,agent from yx_sdk_logininfo where login_time < '".$starttime."' AND agent in({$strSource}) AND userid in({$strUid}) group by agent,userid order by id desc");
			foreach ($before_login_query as $row) {
				$before_login[]=$row;
			}
		}
		...
		$allsourcequery = $model->query("select * from yx_tg_source S left join yx_tg_game G on S.gameid = G.gameid where S.activeflag = 1 and G.activeflag = 1 and G.isonstack = 0 ".$allsource_where." order by S.id desc");
		foreach ($allsourcequery as $row) {
