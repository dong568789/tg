<?php
class apk{
    const AXML_FILE = 524291;
    const STRING_BLOCK = 1835009;
    const RESOURCEIDS = 524672;
    const START_NAMESPACE = 1048832;
    const END_NAMESPACE = 1048833;
    const START_TAG = 1048834;
    const END_TAG = 1048835;
    const TEXT = 1048836;
    const TYPE_NULL = 0;
    ...
    const UNIT_MASK = 15;
    private static $RADIX_MULTS = array(0.00390625, 3.051758E-5, 1.192093E-7, 4.656613E-10);
...
' . str_repeat(' ', $lv);
    ...
    private function parseBlock($need = 0)
    {
        ...
        switch ($type) {
            case self::AXML_FILE:
                $props = array('line' => 0, 'tag' => '<?xml version="1.0" encoding="utf-8"?>');
                break;
            ...
            case self::START_TAG:
                ...
                $o += 4;
                $attrs = array();
                $props = array('line' => $line, 'ns' => $this->getNameSpace($this->get32($o)), 'name' => $this->getString($this->get32($o)), 'flag' => $this->get32($o), 'count' => $this->get16($o), 'id' => $this->get16($o) - 1, 'class' => $this->get16($o) - 1, 'style' => $this->get16($o) - 1, 'attrs' => &$attrs);
                $props['ns_name'] = $props['ns'] . $props['name'];
                for ($i = 0; $i < $props['count']; $i++) {
                    $a = array('ns' => $this->getNameSpace($this->get32($o)), 'name' => $this->getString($this->get32($o)), 'val_str' => $this->get32($o), 'val_type' => $this->get32($o), 'val_data' => $this->get32($o));
            ...
            case self::END_TAG:
                $line = $this->get32($o);
                $o += 4;
                $props = array('line' => $line, 'ns' => $this->getNameSpace($this->get32($o)), 'name' => $this->getString($this->get32($o)));
                $props['ns_name'] = $props['ns'] . $props['name'];
            ...
            case self::TEXT:
                $o += 8;
                $props = array('tag' => $this->getString($this->get32($o)));
                $o += 8;
                break;
            default:
    ...
    {
        return (double) ($data & 4294967040) * self::$RADIX_MULTS[$data >> 4 & 3];
    }
