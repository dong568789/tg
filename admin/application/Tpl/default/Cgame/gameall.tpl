                                ...
                                <div class="table-responsive">
                                    <table id="data-table-basic" class="table table-hover table-vmiddle">
                                        ...
                                        <tr>
                                            ...
                                            <th data-column-id="publisher">发行商</th>
                                        ...
                                        <foreach name="gamelist" item="vo" key="k">
                                            <tr>
												...
												<td><{$vo['gamename']}></td>
                                                <td><{$vo['gamepinyin']}></td>
                                                <td><{$publisher[$vo['publisher']]['name']}></td>
...
<script type="text/javascript">
	...
	function deleteGame (id) {
		...
		if(confirm(confirmcontent)) {
			$.ajax({    
				type : 'POST',        
				url : "index.php?m=Cgame&a=deleteGame",
				data : {gameid : id},
				cache : false,    
				dataType : 'json',    
    ...
    $(document).ready(function() {
        ...
        $("#data-table-basic").bootgrid({
			...
			formatters: {
				"link": function(column, row)
				{
                    if ('<{$editgame}>' == 'ok' && '<{$deletegame}>' == 'ok'){
                        return "<a href=\"/cgame/gamedetail/gameid/"+row.id+"/\">编辑</a>&nbsp;|&nbsp;<a href=\"#\" " +
                                "id=\"delete-"+row
                                        .id+"\" " +
                                "onclick=\"deleteGame("+row.id+");\">删除</a>";

                    } else if('<{$editgame}>' == 'ok' && '<{$deletegame}>' != 'ok'){
                        return "<a href=\"/cgame/gamedetail/gameid/"+row.id+"/\">编辑</a>";
			...
			templates: {
				header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-12 actionBar\"><p class=\"{{css.search}}\"></p><p class=\"{{css.actions}}\"></p><a class=\"{{css.addnewuser}}\" href=\"/cgame/newgame/\">新增一个游戏</a></div></div></div>"
			}
        });
    })
</script>
