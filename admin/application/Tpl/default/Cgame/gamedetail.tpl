            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="p-20">
								<form id="editgame" class="form-horizontal" enctype="multipart/form-data" role="form"
									  action="index.php?m=cgame&a=editgame" method="post" >
												...
												<select class="selectpicker" name="publisher">
													<foreach name="publisher" item="vo" key="k">
														<option value="<{$vo['id']}>" <if
																condition="$game['publisher'] eq  $vo['id']">selected="select"</if>><{$vo['name']}></option>
													</foreach>
												</select>
											</div>
										</div>
					...
					<div class="card">
                        ...
                        <div class="card-body">
							<div class="p-20">
								<form id="uploadpackage" class="form-horizontal" enctype="multipart/form-data"
									  role="form" action="<{:U('cgame/uploadpackage')}>" method="post" >
...
<script type="text/javascript">
	...
	function createForcePackage(isFristTime, nowID) {
		...
		if (parseInt(isfirst) == 1) {
			...
			$.ajax({
				type: "POST",
				url: "index.php?m=cgame&a=createForcePackage",
    ...
    $(document).ready(function(){
								...
								swal({
									...
									closeOnCancel: false
										...
										if (isConfirm) {
											...
											$.ajax({
												type: "POST",
												url: "index.php?m=cgame&a=coverPackage",
		...
		$("#forceupdatesubmit").click(function(){
			...
			if(confirm(confirmcontent)) {
				$.ajax({
					type: "POST",
					url: "index.php?m=cgame&a=forceUpdateCheck",
					data: {packageid : packageid},
					cache: false,
					dataType: 'json',
					success: function (data) {
		...
		$('#repealpackagesubmit').on('click',function(){
			...
			swal({
				...
				if (isConfirm) {
					$.ajax({
						type: "POST",
						url: "index.php?m=cgame&a=repealForce",
						data: {gameid : gameid},
						cache: false,
						dataType: 'json',
						success: function (data) {
