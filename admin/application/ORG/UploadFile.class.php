...
{//类定义开始
     ...
     */
	 ...
	 private function getimageArr($filename){
    ...
    {
		$size_arr= $this->getimageArr($filename);
		$width=$size_arr[0];
		$height=$size_arr[1];
        $rule = $this->saveRule;
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
            ...
            if(function_exists($rule)) {
                //使用函数生成一个唯一文件标识号
				if(($width==64 && $height==64) || ($width==80 && $height==98) || ($width==110 && $height==135) ){
					$saveName = $rule()."_".$width."X".$height.".".$filename['extension'];
				} else{
